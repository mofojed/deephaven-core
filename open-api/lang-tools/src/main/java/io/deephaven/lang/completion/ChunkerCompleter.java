...
public class ChunkerCompleter implements CompletionHandler {
    ...
    private void annotationComplete(Collection<CompletionItem.Builder> results, ChunkerAnnotation node,
    ...
    private void assignCompletion(Collection<CompletionItem.Builder> results, ChunkerAssign node,
            CompletionRequest offset) {
    ...
    private void typedAssignCompletion(Collection<CompletionItem.Builder> results, ChunkerTypedAssign node,
            CompletionRequest offset) {}

    private void typeParamsCompletion(Collection<CompletionItem.Builder> results, ChunkerTypeParams node,
            CompletionRequest offset) {}

    private void typeParamCompletion(Collection<CompletionItem.Builder> results, ChunkerTypeParam node,
            CompletionRequest offset) {
    ...
    public List<ChunkerAssign> findAssignment(final ParsedDocument doc, final CompletionRequest request,
            final String name) {
    ...
    private void maybeColumnComplete(
        ...
        if (doColumnNameCompletion) {
            if (def == null) {
                ...
                log.info().append("No table definition found for ").append(String.valueOf(invoke.getScope()))
                        .append(" at offset ").append(offset.toString()).endl();
            } else {
                ...
                for (Class cls : lookups.getStatics()) {
                    for (Method method : cls.getMethods()) {
                        ...
                        if (colType.isAssignableFrom(method.getReturnType())) {
                            ...
                            // TODO: check binding for variables that can be referenced in column expressions (any
                            // non-typed assign before now) IDS-1517-23
    ...
    @Deprecated
    public void addMatch(Collection<CompletionItem.Builder> results, Node node, String match, CompletionRequest index,
            String... nextTokens) {
