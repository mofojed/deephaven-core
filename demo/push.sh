...
while [ -n "$dir" ]; do
  if [ -f "$dir/gradlew" ]; then
    ...
    docker rmi us-central1-docker.pkg.dev/deephaven-oss/deephaven/grpc-api:0.7.02 &>/dev/null
    docker rmi us-central1-docker.pkg.dev/deephaven-oss/deephaven/web:0.7.02 &>/dev/null
