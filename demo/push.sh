...
while [ -n "$dir" ]; do
  if [ -f "$dir/gradlew" ]; then
    ...
    docker rmi us-central1-docker.pkg.dev/deephaven-oss/deephaven/grpc-api:0.5.15 &>/dev/null
    docker rmi us-central1-docker.pkg.dev/deephaven-oss/deephaven/web:0.5.15 &>/dev/null
